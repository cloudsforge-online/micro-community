...
# `runtime`.
#
# This service takes BOTH: `src/ledgerclient.ts` and `src/executions.ts` import
# `@cloudsforge/contracts-money` for `AccountSubject`, `Actor` and `LedgerAssetCode`, so the
...
# nothing at runtime needs them. What this container can do is execute a treasury spend against
# micro-ledger, which is the argument for keeping its attack surface as small as it will go.
#
...
# time. There is no ENV line here on purpose — least of all COMMUNITY_SERVICE_CREDENTIAL, which is
# the credential this service presents to the ledger, or OUTBOX_SIGNING_SECRET.
#
# There is also no variable that could shorten or bypass a timelock, because no such variable exists
# anywhere in this service. See src/env.ts and .env.example.
...
# SCHEMA_VERSION the `executions_respect_timelock` trigger and the `votes_proposal_subject_uniq`
# constraint may not exist, and a service that could create them at boot is a service that could
# start without them — which is a service that executes treasury spends before their timelock and
# counts a member's vote twice. `index.ts` asserts the schema version and refuses to serve below it.
