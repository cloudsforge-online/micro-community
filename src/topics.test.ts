 ...
 * share this namespace. A scope such as `community.proposal.write` (`scopes.ts`) is three
 * lowercase segments and looks exactly like a topic, and nothing emits it.
 */
...
test('every registry topic this service owns is actually emitted', () => {
  ...
  assert.deepEqual(
    ...
    [],
    'the registry says community produces these and no emit site does — every consumer of each is dead code',
  )
  // And the registry is being read rather than the check passing vacuously.
  assert.ok(topicsProducedBy(SERVICE).length >= 3, 'the registry is being read, not passing vacuously')
...
test('a pending proposal disappears once contracts adopts it', () => {
  ...
  // entry stays, and the next reader believes the topic is still unregistered. `events.ts` used
  // to make exactly that claim in prose, and it had been false for as long as it took a reader
  // to check.
  assert.deepEqual(
    adoptedProposals(),
    [],
  ...
  // The three the registry DOES name must not be quarantined, and this is what fails the day a
  // fourth is adopted.
  for (const registered of [
    ...
    assert.equal(isRegisteredTopic(registered), true, `${registered} is registered`)
    assert.equal(Object.hasOwn(AWAITING_REGISTRATION, registered), false)
  }
  assert.equal(topicsProducedBy(SERVICE).length, 3)
})
...
test('every pending proposal carries a spec that could be pasted into the registry', () => {
  assert.deepEqual(
    malformedProposals(),
    [],
    'a proposal needs a well-formed community topic, a real ordering key, and a reason worth reading',
  )
})
...
const ROW = {
  ...
  topic: 'community.proposal.opened',
  ...
  payload: { proposalId: 'p-1', communityId: 'c-1' },
}
...
test('a row with no actor and no correlation id still makes a readable envelope', () => {
  ...
  assert.equal(envelope.actor, 'service:community')
...
test('envelopeDefects excuses a lagging registry and nothing else', () => {
  ...
  const quarantined = buildEnvelope({ ...ROW, topic: 'community.proposal.closed' })
  ...
  const unexplained = { ...buildEnvelope(ROW), topic: 'community.nothing.happened' }
...
test('the delivery this relay signs is one a contract-following consumer verifies', () => {
  // The other half of the same story, and THIS SERVICE carried the drifted copy until now:
  // `x-cloudsforge-signature` with `sha256=<hmac over the body>`. Every delivery to activity or
  // notify was refused as "signature: missing", and the timestamp that makes the freshness window
  // mean anything was not in the signed material at all.
