 ...
 *
 * It matters concretely here: below `SCHEMA_VERSION` the `executions_respect_timelock` and
 * `votes_proposal_subject_uniq` objects may not exist, and a service that could create them at
 * boot is a service that could start without them — which is a service that executes treasury
 * spends before their timelock and counts a member's vote twice. `index.ts` asserts the version
