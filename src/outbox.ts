 ...
 * commit and the transaction rolls back, every consumer believes a spend happened that did not.
 ...
 *
 *   const order = await withOutbox(sql, SERVICE, async (tx, emit) => {
 *     const row = await insertOrder(tx, input)
 *     emit({ topic: 'market.listing.sold', key: row.listingId, payload: { … } })
 *     return row
 *   })
 *
 ...
 *
 * `withOutbox` above is the right shape when the whole operation fits in one closure. Most of this
 * service's writes do not: a settlement locks a listing, calls the ledger, captures two escrows
 * and writes an order, and it needs the transaction handle throughout. So those open their own
 * `sql.begin` and call this, which is the same INSERT with the same guarantee — the event and the
