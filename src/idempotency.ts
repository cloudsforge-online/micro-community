 ...
 *
 * The key is namespaced by the **principal**, not by the calling service, and here that matters
 * more than it does in the money services: this surface is used by people rather than by
 * machines, and two members of the same community independently choosing `vote-1` is not a
 * hypothetical. Namespaced by service alone, the second would be answered with the first's
 * response — which on `POST /v1/proposals/:id/votes` means being told your vote was recorded when
 * what was recorded was somebody else's.
 *
 * ══════════════════════════════════════════════════════════════════════════════════════════════
 ...
 * genuinely different choice still 409s.
 *
...
/**
 * The stored key, namespaced by the principal and the route.
 *
 * The route is in the key because the same client key presented to `POST /v1/communities` and to
 * `POST /v1/proposals/:id/votes` describes two different operations, and a caller reusing its
 * request id across both is doing something reasonable. The principal is in it for the reason in
 * the header: two members choosing the same key must not be answered with each other's response.
 */
...
export interface IdempotencyInput<T> {
  /** `user:<id>`, or `service:<name>` on an internal route. Never empty. */
   ...
   * the claim row points at the proposal, vote or execution and an operator can join a caller's
   * key to what it produced.
   */
 ...
 *
 * The cutoff is the entire safety argument: expiring a key EARLY means the next replay of it runs
 * the work a second time, so the TTL has to outlive every caller's retry horizon rather than be
 ...
 * caller's key and the proposal or execution it made, and losing it turns "did my retry spend the
 * treasury twice" into an unanswerable question.
 */
