...
export async function createCommunity(
  ...
  const rows = await tx<CommunityRow[]>`
    insert into communities (slug, name, kind, owner_subject, join_policy, treasury_subject,
    ...
    values (${input.slug}, ${input.name}, ${input.kind}, ${input.ownerSubject}, ${input.joinPolicy},
            -- Deliberately a placeholder that the UPDATE below replaces: \`treasury_subject\` is
            -- 'community:' || id and the id is generated by this INSERT, so it cannot be written
            -- in the same statement. The CHECK is what makes the two-step safe — a transaction
            -- that forgot the UPDATE could not commit.
            'community:pending', ${input.governanceModel},
  ...
  if (!created) throw new ValidationError('the community row was not written')

  const settled = await tx<CommunityRow[]>`
    update communities
       set treasury_subject = 'community:' || id::text
     where id = ${created.id}
    returning ${tx.unsafe(COMMUNITY_COLUMNS)}
  `
  const row = settled[0]
  if (!row) throw new ValidationError('the community row vanished mid-transaction')
